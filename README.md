...
Полученную модель, код и отчет залить в папку этого репозитория **students/группа/laboratory_works/фамилия_имя/laboratory_work_1**. Инструкция о загрузке работы ниже. Не забывайте о файле гитигнор.
На гит должен быть загружен пдф файл с отчетом, код программамы, **где каждая папка соотвествует часть работы (task_1, Task_2...)**. 
