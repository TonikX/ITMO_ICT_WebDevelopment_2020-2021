  ...
  <section>
    <v-row>
      <v-col cols="6" class="mx-auto">
        <epic-item-card v-for="epicItem in epicItems" :key="epicItem.identifier" :epic-item="epicItem" class="my-2" />
      </v-col>
    </v-row>
  </section>
</template>
  ...
  methods: {
    async getEpicItems() {
      try {
        ...
        // фотографий земли
        const response = await this.axios.get(`${apiUrl}/api/natural?api_key=${apiKey}`);
