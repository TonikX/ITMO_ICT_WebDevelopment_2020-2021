...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Type_material', value: 'type_service' },
      { text: 'Description', value: 'desc' },
      { text: 'Actions', value: 'actions', sortable: false }
    ],
    materials: [],
    type_serviceList: ['banner', 'film', 'paper'],
  ...
  methods: {
    async materialsList () {
      try {
        const response = await axios
          .get('http://127.0.0.1:8000/luch/material/list')
    ...
    deleteItemConfirm () {
      if (this.editedIndex > -1) {
        ...
        this.closeDelete()
        axios.delete('http://127.0.0.1:8000/luch/material/' + JSON.stringify(this.editedItem.id), {
    ...
    save () {
      if (this.editedIndex > -1) {
        ...
        axios.put('http://127.0.0.1:8000/luch/material/' + JSON.stringify(this.editedItem.id), {
          id: this.editedItem.id,
          ...
          .catch(error => {
            console.log(error)
            axios.post('http://127.0.0.1:8000/luch/material/create/', {
              breed: this.editedItem.breed,
              productivity: this.editedItem.productivity,
              avg_weight: this.editedItem.avg_weight,
              diet: this.editedItem.diet
