...
def confsslist(request):
    visual = {"confs": Conf.objects.all()}
    return render(request, 'conf.html', visual)
...
class CreateReserve(CreateView):
    ...
    def get_initial(self):
        ...
        initial['name'] = get_object_or_404(Conf, pk=self.kwargs['pk'])
        return initial


def reservedconfslist(request):
    visual = {"reserves": Reserve.objects.all()}
    return render(request, 'reservedconfs.html', visual)
