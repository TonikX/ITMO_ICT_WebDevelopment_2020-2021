...
class UpdateSkillSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        warrior = Warrior.objects.create(skill=validated_data['skill'])

        for option in validated_data['skill']:
            skill_update = SkillOfWarrior(skill=option['skill'], level=option['level'])
            skill_update.save()
